...
export default function Shell({
  ...
  return (
    <div className="min-h-screen flex bg-[#05070d]">
      <aside className="hidden lg:flex w-72 shrink-0 border-r border-slate-800/80 bg-[#070a13] flex-col">
        <div className="px-5 pt-6 pb-4 border-b border-slate-800/80">
          <Link href="/" className="flex items-center gap-2 group">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-sky-400 to-indigo-500 flex items-center justify-center text-slate-900 font-black text-sm glow">
              S
            </div>
            <div className="leading-tight">
              ...
              <div className="text-[10px] uppercase tracking-widest text-sky-400/80">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-6">
          {NAV_SECTIONS.map((section) => (
            <div key={section.title}>
              <div className="px-2 pb-2 text-[10px] uppercase tracking-widest text-slate-500 font-semibold">
                {section.title}
              </div>
                  ...
                  return (
                    <li key={item.href}>
                      ...
                      >
                        {item.label}
                        {item.badge && (
                          <span className="ml-2 text-[9px] uppercase tracking-widest text-sky-400">
                            {item.badge}
                          </span>
                        )}
                      </Link>
        ...
        <div className="px-4 py-3 border-t border-slate-800/80 text-[10px] text-slate-500 leading-relaxed">
          Passive by default · Active requires verified target + explicit
          opt-in · Every request scope-checked in-worker.
        </div>
      </aside>
      <main className="flex-1 min-w-0">
        <div className="lg:hidden sticky top-0 z-10 border-b border-slate-800 bg-[#070a13]/80 backdrop-blur px-4 py-3 flex items-center gap-2">
          <div className="w-7 h-7 rounded-md bg-gradient-to-br from-sky-400 to-indigo-500 flex items-center justify-center text-slate-900 font-black text-xs">
            S
          </div>
          <span className="font-semibold text-slate-100">SentinelDAST</span>
          <Link href="/spec" className="ml-auto text-xs text-sky-400 hover:underline">
            spec
          </Link>
        </div>
        {children}
...
export function PageHeader({
  ...
  return (
    <div className="border-b border-slate-800/80 bg-gradient-to-b from-[#0a0f1c] to-transparent">
      <div className="max-w-6xl mx-auto px-6 lg:px-10 pt-10 pb-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            {eyebrow && (
              <div className="text-[11px] uppercase tracking-[0.2em] text-sky-400/90 font-semibold mb-2">
                {eyebrow}
              </div>
            )}
            ...
            {description && (
              <p className="mt-2 text-slate-400 max-w-3xl">{description}</p>
            )}
          </div>
...
export function SectionCard({
  ...
  return (
    <section
      className={`rounded-xl border border-slate-800 bg-[#0a0f1c]/60 backdrop-blur ${className}`}
    >
      {(title || eyebrow) && (
        <header className="px-5 py-4 border-b border-slate-800/80">
          {eyebrow && (
            <div className="text-[10px] uppercase tracking-widest text-sky-400/90 font-semibold mb-1">
              {eyebrow}
            </div>
          )}
          {title && (
